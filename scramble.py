...
class Translator:
    def __init__(self, fin, fot, hot, name, egg, guard_prefix = "", dot = None):
        ...
        self.guard = guard_prefix + "_" +\
            name.upper().replace("/", "_").replace(".", "_") + "_"
...
""".lstrip() % (self.blah,
        ...
        self.name))
        if options.use_include:
            self.to.write("""#include "scramble.h"\n""")
        else:
            self.to.write("""\
#undef None
#undef min
...
""".lstrip() % (self.blah,
    ...
    def add_to_docstring(self, l):
        ...
        if self.docstring.rstrip().endswith('"""'):
            ...
            if options.comments and doc:
                docl = doc.splitlines()
                docc = "\n".join([docl[0]] + ["     * " + x for x in docl[1:]])
                self.to.write("    /* %s */\n" % docc)

            self.docstring = None

            if self.numbering: self.to.write("#line %d // doc\n" % (
                self.num + 1))
        ...
        for l in self.fin.readlines() + ["end:"]:
            ...
            if c >= 0:
                ...
                if not tokens[0] in ["#if", "#ifdef", "#ifndef", "#endif", "#else",
                    ...
                    "#both", "macro"]:
                    if len(tokens) <= 1 or not (tokens[0] in ["static",
                        "global"] and tokens[1] in ["macro"]):
                        if options.comments:
                            self.to.write(
                                " " * self.depths[-1].depth +
                                "/* %s */\n" % l[c + 1:].strip())
    ...
    def handle_line(self, l):
        ...
        if l[-1] == ":":
            if l == "end:":
                ...
                self.translate_class(l)
            elif re.compile(r"static enum\b").match(l):
                params = l[11:-1].strip()
                self.to.write("typedef enum %s %s;\n" % (params, params))
                self.to.write("enum %s\n" % params)
                self.depths[-1].is_enum = True
            elif re.compile(r"enum\b").match(l):
                params = l[4:-1].strip()
                self.typedefs.write("#ifndef __cplusplus\n")
                self.typedefs.write("typedef enum %s %s;\n" % (params, params))
                self.typedefs.write("#endif\n")
    ...
    def translate_def(self, l):
        ...
        if not mob:
            ...
            params = ", ".join(params2)
            if not static:
                if self.numbering: self.hot.write("#line %d // def\n" % self.num)
    ...
    def translate_import(self, t, names):
        ...
        for name in names:
            ...
            if name.startswith("global "):
                ...
                name = name[7:].strip()
            if name:
                if self.numbering: t.write("#line %d // import\n" % self.num)
...
def main():
    ...
    parser.add_option("-C", "--comments", help="Retain comments in the output.",
        action = "store_true")
    ...
    parser.add_option("-U", "--use-include", help="Use scramble.h instead of pasting it.",
        action = "store_true")
